...
function Game() {
  ...
  const specialAttackHandler = () => {
    const playerDamage = getRandomValue(20, 35); 
    const monsterDamage = getRandomValue(8, 15); 
    setMonsterHealth((prev) => Math.max(prev - playerDamage, 0));
    setPlayerHealth((prev) => Math.max(prev - monsterDamage, 0));
    setTurnCount((prev) => prev + 1);
  ...
  const healHandler = () => {
    const healing = getRandomValue(10, 18); 
    const monsterDamage = getRandomValue(5, 12); 
    setPlayerHealth((prev) => {
      const afterHeal = Math.min(prev + healing, 100); 
      return Math.max(afterHeal - monsterDamage, 0); 
    });
    setTurnCount((prev) => prev + 1);
    setLogs((prev) => [
